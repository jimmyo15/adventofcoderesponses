...
class BingoGame:
    def __init__(self):
        ...
        self.worst_board = None
        self.worst_board_moves_count = 0
        self.worst_board_selected_sum = None
    ...
    def find_worst_board(self, input_file):
        board_counter = 0
        self.set_number_order(input_file)
        next_line = input_file.readline()
        while next_line:
            ...
            print(board_counter)
            new_board = self.build_board(input_file)
            self.analyze_worst_board(new_board)
            next_line = file.readline()

    def analyze_worst_board(self, board):
        num_counter = 0
        selected_row_col = dict()
        selected_row_col['row'] = dict()
        selected_row_col['col'] = dict()
        ...
        for number in self.number_order:
            ...
            if location:
                ...
                selected_numbers_sum += number
                if len(selected_row_col['row'][location[0]]) == 5:
                    if num_counter >= self.worst_board_moves_count:
                        self.worst_board = board
                        self.worst_board_moves_count = num_counter
                        self.worst_board_selected_sum = selected_numbers_sum
                    return
                if len(selected_row_col['col'][location[1]]) == 5:
                    if num_counter >= self.worst_board_moves_count:
                        self.worst_board = board
...
if __name__ == "__main__":
    ...
    bg = BingoGame()
    bg.find_worst_board(file)
    print(bg.worst_board.board_all_numbers_sum)
    print(bg.worst_board_selected_sum)
    print(bg.worst_board_moves_count)
    print(bg.number_order[bg.worst_board_moves_count - 1])
    print(bg.worst_board.board_all_numbers_sum -
          bg.worst_board_selected_sum)
    file.close()
